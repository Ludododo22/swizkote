...
export default function DashboardPage() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {!(user as any)?.active && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/40 text-red-500" data-testid="banner-account-inactive">
          <div className="w-8 h-8 rounded-full bg-red-500/15 flex items-center justify-center flex-shrink-0">
            <Shield className="w-4 h-4 text-red-500" />
          </div>
          <div>
            <p className="text-sm font-semibold">{t("account_inactive_title")}</p>
            <p className="text-xs text-red-400 mt-0.5">{t("account_inactive_desc")}</p>
          </div>
        </div>
      )}
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-2 pb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">{de_lang ? "Überweisungshistorie" : "Historique des transferts"}</span>
          </div>
          <Link href="/transfers">
        ...
        <CardContent>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  {de_lang ? "Ausstehende Überweisungen" : "Virements en attente"}
                </span>
                <Badge variant="secondary" className="text-xs">{activeTransfers.length}</Badge>
              </div>
              {activeTransfers.length > 0 ? (
                <div className="space-y-1">
                  {activeTransfers.slice(0, 3).map(tf => (
                    <div key={tf.id} className="space-y-1">
                      <Link href={`/transfers/${tf.id}`}>
                        <div className="flex items-center justify-between gap-3 p-2.5 rounded-lg hover-elevate cursor-pointer">
                          <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                              <p className="text-sm font-medium truncate">{tf.recipientName}</p>
                              <p className="text-xs text-muted-foreground">{tf.createdAt ? format(new Date(tf.createdAt), "d MMM yyyy", { locale }) : ""}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <span className="text-sm font-semibold text-amber-500">{formatCurrency(tf.amount, tf.currency)}</span>
                          </div>
                        </div>
                      </Link>
                      <div className="pl-2 pb-1">
                        <Link href={`/transfers/${tf.id}`}>
                          <button className="text-xs text-gold hover:underline flex items-center gap-1 px-2">
                            <ArrowUpRight className="w-3 h-3" />
                            {de_lang ? "Überweisung verfolgen" : "Suivre le virement"}
                          </button>
                        </Link>
                      </div>
                    </div>
                ...
                <p className="text-xs text-muted-foreground text-center py-3">{de_lang ? "Keine ausstehenden Überweisungen" : "Aucun virement en attente"}</p>
              )}
            </div>
            ...
            <div className="border-t pt-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  {de_lang ? "Abgeschlossene Überweisungen" : "Virements effectués"}
                </span>
                <Badge variant="secondary" className="text-xs">{completedTransfers.length}</Badge>
              </div>
              {completedTransfers.length > 0 ? (
                <div className="space-y-1">
                  {completedTransfers.slice(0, 3).map(tf => (
                    <Link key={tf.id} href={`/transfers/${tf.id}`}>
                      <div className="flex items-center justify-between gap-3 p-2.5 rounded-lg hover-elevate cursor-pointer">
                        <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <span className="text-sm font-semibold text-green-500 flex-shrink-0">{formatCurrency(tf.amount, tf.currency)}</span>
                      </div>
                    </Link>
                  ))}
                </div>
              ) : (
                <p className="text-xs text-muted-foreground text-center py-3">{de_lang ? "Keine abgeschlossenen Überweisungen" : "Aucun virement effectué"}</p>
              )}
            </div>
          </div>
        </CardContent>
